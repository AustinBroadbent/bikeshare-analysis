...
This case study was completed as part of Capstone for the Google Data Analytics Professional Certificate. The purpose of this project is to showcase technical skills (SQL, R, and Python) as well as soft skills (including problem-solving, attention to detail, and data communication). Alongside this notebook, I created a [Google Slides presentation](https://docs.google.com/presentation/d/1clFMFjLqcbdR5OpM11Mj60wg4BDLJ0ZY8kuD1LvNmzs/edit?usp=sharing) for a non-technical communication of the main insights.

## 1.1 Project Background & Objectives

Cyclistic is a (fictional) bikeshare program that features more than 5,800 bicycles and 600 docking stations around Chicago. The company offers single-ride passes, full-day passes, and annual memberships. Within the company, customers who purchase single-ride or full-day passes are referred to as casual riders, while customers who purchase annual memberships are Cyclistic members. More info on the case study can be found [here](https://drive.google.com/file/d/14vff1pCBKbTh6yTi_PjBCjc1rhFSK-HD/view?usp=sharing).

This analysis will focus on the past 12 months of Cyclistic historical bike ride data (March 2024 - February 2025. The aim is to understand the differences in how casual riders and annual members use Cyclistic bikes. The results of this analysis will be used to design marketing strategies aimed at converting casual riders into annual members.

## 1.2 Data Source & Description

The [data source](https://divvy-tripdata.s3.amazonaws.com/index.html) consists of Cyclistic's historical trip data from March 2024 to February 2025. The data for each month of the time period was originally contained within a CSV file (12 CSVs in total). Key variables for our analysis include:
...
As stated, the original data was split into 12 CSV files: one for each month. I created a table in BigQuery called `bike_trip_data`. I appended each CSV to the end of the table, aggregating all of the CSVs into one queryable object. 
> The first snag that I encountered was that the free version of BigQuery only allows files up to 100MB to be uploaded. So I went into Python and wrote a function to split the larger CSVs into multiple files:
...
I verified that there was no loss of data when running the `split_csv()` function [(Kaggle Notebook)](https://www.kaggle.com/code/austinbroadbent/split-csvs) or during upload. The resulting table contained 5,783,100 observations and 13 columns:

![](data/images/bike-trip-data.png)

## 2.1 Missing Value Handling

Now that the data was aggregated and in one place, the first thing I noticed was that there were many missing values. 

> Missing values existed in `start_station_name`, `start_station_id`, `end_station_name`, `end_station_id`, `end_longitude`, and `end_latitude`.
...
missing_df <- read_csv("C:/Users/Administrator/Downloads/missing_values.csv", show_col_types = FALSE)
missing_df <- t(missing_df)
knitr::kable(missing_df)
```

Since all of these columns are variables dealing with location data, and the numbers of missing values appear in pairs, I investigated to confirm the following relationships:

1.  When the `start_station_name` was missing from an observation, the `start_station_id` was also missing.

2.  The same was true for `end_station_name` and `end_station_id`.

3.  If `end_longitude`, and `end_latitude` were missing, `end_station_name` and `end_station_id` were also missing. In other words, the missing GPS data was a subset of the missing end station data.

4.  There were 527,697 rows missing both `start_station` and `end_station` data. None of these are included in the subset of missing GPS data.

In a real scenario, I would investigate further to try and find the cause of the missing data. It could be the result of errors in recording the station information, problems with the data collection process, GPS errors, software/hardware malfunctions, etc.

> **But, for this case study a key part of my analysis will rest on the trip duration. Therefore, I felt that the missing location data called into question the reliability of these observations. Therefore I decided to filter out these observations and save the results into a new table. This way, the missing data is still contained in the original table and still easily accessible.**
...
```{r duplicate_values, echo=FALSE}
duplicates_df <- read_csv("C:/Users/Administrator/Downloads/duplicate_rows.csv", show_col_types = FALSE)
knitr::kable(head(duplicates_df))
```

Note that there were 121 duplicates. I noticed that all of the duplicates had a count of 2. Upon further investigation, the duplicated instances were identical, EXCEPT that the start and end times for one instance were more precise than the other instance. Below is an example case:

```{r example_duplicate, echo=FALSE}
example_duplicate <- read_csv("C:/Users/Administrator/Downloads/example_duplicate.csv", show_col_types = FALSE)
knitr::kable(example_duplicate)
```

> I decided that the more precise time was likely to be more accurate. For each pair of duplicates, I kept the record with the more detailed time stamp and filtered out the other, making a third table.
...
Next, I wanted to check for inconsistencies with the start and end times of the rides. 
> I added a new column to calculate the ride duration in minutes. This needed two SQL queries: an `ALTER TABLE` query to add the column, and an `UPDATE` to populate it with the calculation.
...
```{r long-histogram, echo=FALSE, out.width="100%"}
knitr::include_graphics("data/images/long-histogram.png")
```
...
```{r short-histogram, echo=FALSE, out.width="100%"}
knitr::include_graphics("data/images/long-histogram.png")
```

------------------------------------------------------------------------

Just as with the missing location data, further investigation could be done to determine a cause for these outliers. As is, I found some interesting anomalies. 

* Between 4-5AM, there was a huge increase in extremely short rides (\<15 seconds), as well as rides that are abnormally long (\>400 minutes). This suggest there could be a system error occurring at this time that affects the correct logging of start/end times.

Since I lacked information, I decided to be conservative in my approach. 
> I filtered out rides that were less than 15 seconds (0.25 minutes), and those that were greater than 16 hours (960 minutes or the time that most people are awake in a day). This was done using the same SQL query as removing negative ride durations, just adjusting the inequality and integer.
...
Once the data was ready, I uploaded it to RStudio because R make data visualization easy. Since I decided not to include location data in the analysis, I filtered the data frame for the relevant columns:
...
* **Summary Statistics:** Calculating descriptive statistics such as mean, median, standard deviation, minimum, and maximum values to get a basic understanding of the distribution of ride duration
...
To make these visualizations easier to accomplish, I used the `started_at` column to create new features `hour`, `weekday`, and `month`. After creating the `weekday` column, I wanted the weekdays to be plotted in order starting with Monday so that I could more easily compare weekends vs. weekdays. Thus, I converted the column to an ordered factor.
...
Descriptive statistics were calculated to provide a summary of `ride_duration_minutes` for casual riders and annual members. This helped to see at a quick glance the basic distribution of each group:
...
This section presents the final visualizations I made to explore the patterns in Cyclistic's ride data. I will provide a caption to explain the purpose and key insights gained from each plot. 

> Note: the visual design choices were made to match the theme of the Google Slides presentation I made to summarize the analysis. The final results of that presentation are [here](https://docs.google.com/presentation/d/1clFMFjLqcbdR5OpM11Mj60wg4BDLJ0ZY8kuD1LvNmzs/edit?usp=sharing).)
...
This bar chart compares the total number of rides taken by casual riders and annual members during the past 12 months. It shows that members take the majority of rides (63.56%, in fact).

``` {r rider_type, message=FALSE, fig.width=10}
...
This bar chart compares the average ride duration (in minutes) for casual riders and annual members. It helps to understand the typical trip length for each rider group. While members take more trips, casual riders take longer trips, on average. (almost twice as long!)

``` {r total_avg, fig.width=10}
...
These side by side histograms compare the time of day when each rider type takes their rides. It shows that members have peak usage during commuting hours, while casual riders during midday and afternoon hours.

``` {r hourly_rider_type, message=FALSE, fig.width=10}
...
This line chart shows the average ride duration by hour of the day. It shows that casual riders have longer averages overall (which matches our expectations from the first bar chart). But also, casual riders have more varied ride times throughout the day, while members' average ride duration stays fairly steady.

``` {r hourly_avg, warning=FALSE, fig.width=10}
...
These side by side histograms show the distribution of rides by day of the week for each group. We can see an uptick in the number of rides by casual riders on the weekend, while the members have the highest usage during the weekdays.

``` {r weekday_rider_type, message=FALSE, fig.width=10}
...
This line chart shows the average ride duration by day of the week. It shows that both types of riders also have longer average ride times on weekends than weekdays, but this effect is more pronounced for casual riders.


``` {r weekday_average, fig.width=10}
...
These side by side histograms show the distributions of rides by month. It can help us see the seasonal trends in rider usage. It shows that both groups use the service more in warmer months, from about May until October.

``` {r monthly_rider_type, message=FALSE, fig.width=10}
...
This line chart shows the average ride duration by month. It shows that both members use the service for longer in the warmer months, but as with weekdays, this effect is more pronounced in casual riders.

``` {r monthly_average, fig.width=10}
...
``` {r rideable_type, message=FALSE, fig.width=10}
...
This section goes into more depth about the analysis results, directly compares casual riders and members, and addresses the limitations of the analysis.

## 5.1 Interpretation of Results

The analysis shows differences in how casual riders and annual members use Cyclistic bikes.

* Annual members are frequent users, using Cyclistic for their daily routine. This is supported by the higher number of rides taken by members and the peak usage during commuting hours on weekdays.
...
* There was not much difference in the preference of bike types. Both rider groups preferred classic and electric bikes over electric scooters.
...
| Seasonal Use        | Stronger increase in warmer months    | Increase in warmer months             |
| Purpose             | Recreation, Leisure                   | Commuting, Daily Routine              |
...
* **Marketing Focus:** Emphasize the recreational benefits of Cyclistic in marketing campaigns, highlighting the enjoyment of longer rides and city exploration. These marketing campaigns can take place on social media, on sponsored blog posts, or through user-generated content.
...
* **Imprecise GPS Data:** A further subset of the data had imprecise latitude and longitude data, which could indicate faulty data collection. This was further reason why the geographical analysis was not performed.
...
* Members take more rides, suggesting Cyclistic is part of their daily routine, perhaps in commuting to work.
* Casual riders take longer rides, showing a preference for recreational use.
* Members have maximum usage during commuting hours on weekdays, while casual riders have maximum usage during midday and afternoons, especially on weekends.
...
* The differences found in this analysis are to be used for developing targeted marketing strategies to convert casual riders into annual members.
...
* [Case Study Background Information](https://drive.google.com/file/d/14vff1pCBKbTh6yTi_PjBCjc1rhFSK-HD/view?usp=sharing)
* [Original Data Source](https://divvy-tripdata.s3.amazonaws.com/index.html)
* [Google Slides Presentation](https://docs.google.com/presentation/d/1clFMFjLqcbdR5OpM11Mj60wg4BDLJ0ZY8kuD1LvNmzs/edit?usp=sharing)
